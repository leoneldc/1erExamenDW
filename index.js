...
function randomPokemon() {
  ...
  listadoRandom()
    .then(async function (data) {
      ...
      const random = Math.floor(Math.random() * 387);
    ...
    .then(async function (response) {
      ...
      const urlImg = dataJsn.sprites.other.dream_world.front_default;
      var divImgs = `<div class="imagenes"><img src="${urlImg}" alt="${nombre}"></div>`;
      p.innerHTML += `${divImgs}`;
...
async function cargarURLS(pokemon) {
  ...
  divBoton.classList.add("ver_mas");
  ...
  pokemonData.types.map((types) => {
    ...
    datosFormato = `<div style="color: ${
      colorsLetras[types.type.name]
    };"  class="nombre">${nombre.toUpperCase()}</div>
                      <div class="id">${id}</div>`;
  ...
  if (pokemonData.sprites.other.dream_world.front_default !== null) {
    ...
    imagen = `<img src="assets/pokemon_desconocido.png" alt="${nombre}" class="img_pokemon">`;
  }

  botonVer = `<button class="botonVer" >VER MÁS...</button>`;
  //onclick="verDescripcion()"
  divDatos.innerHTML = `${datosFormato}`;
  divTipos.innerHTML = `${tipo}`;
  divImagen.innerHTML = `${imagen}`;
  divBoton.innerHTML = `${botonVer}`;
...
let pokemonGeneration = generaciones(91);
randomPokemon();
